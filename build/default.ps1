...
Task Build-Solution -Depends Clean, Generate-VersionInfo {
	Exec { msbuild "$baseDir\Loader.sln" /verbosity:minimal /p:"Configuration=$configuration" }
}

Task Build-NuGetPackages -Depends Determine-Version, Build-Solution {
	$config = [xml](Get-Content $baseDir\Loader\packages.config)
...
<package xmlns="http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd">
	...
	<files>
		...
		<file src="$baseDir\Loader\bin\Saltarelle.Loader.dll" target="lib"/>
		<file src="$baseDir\Loader\bin\Saltarelle.Loader.xml" target="lib"/>
		<file src="$baseDir\Loader.Script\bin\ssloader.js" target=""/>
		<file src="$baseDir\Loader.Script\bin\ssloader.min.js" target=""/>
	</files>
</package>
"@ | Out-File -Encoding UTF8 "$outDir\Loader.nuspec"

	Exec { & "$buildtoolsDir\nuget.exe" pack "$outDir\Loader.nuspec" -NoPackageAnalysis -OutputDirectory "$outDir" }
}
...
Task Generate-VersionInfo -Depends Determine-Version {
	Generate-VersionFile -Path "$baseDir\Loader\Properties\Version.cs" -Version $script:Version
}
